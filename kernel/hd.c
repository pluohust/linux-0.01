...
void get_hd_param(void)
{
	struct hd_driveid *id;
	unsigned short buf_test[256];
...
//	nobusy();
	...
	outb(0xec,0x1f7);
     	nobusy();
	insw(HD_DATA,id,512<<1);
        printk("cyls is %d!,heads is %d!,sectors is %d!\n",id->cyls,id->heads,id->sectors); 
...
/*	outb(0xa0,0x1f6);
	...
	outb(0x20,0x1f7);
		...
		delay_my();
     	...
     	port_read(HD_DATA,id,512<<1);
        printk("cyls is %x!,heads is %s!,sectors is %x!\n",id->cyls,id->heads,id->sectors); 
	
*/	
	// 	hd[0].start_sect=0;
    
	//   	hd[0].nr_sects=hd_info[0].head*hd_info[0].cyl*hd_info[0].sect;
...
static void reset_controller(void)
{
	...
	printk("reset_controller is called!");
	outb(4,HD_CMD);
...
//static	struct hd_driveid *id;
	...
	nobusy();
        OUT_BYTE(0Xa0,0X1F6);
	OUT_BYTE(0xec,0x1f7);		/* ask drive for ID */
	...
	printk("\nYour hd CHS(physical):=%d/%d/%d!\n",id->cyls,id->heads,id->sectors);
//	printk("and the logical param:CHS=%d/%d/%d/%d!\n ",id->cur_cyls,id->cur_heads,id->sectors,id->lba_capacity);
	sti();
}
